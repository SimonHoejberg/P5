...
setup, \fix{only functions with a return value is tested}{You ought not have
others?!Out Values?Stubs/mocks?} and functions which uses input from the sensors are not tested. The basic principle is Arrange, Act and
...
to the expected we return true. This return value is then used to \fix{print the
value on the screen of the NXT to have a way}{log fil?}of representing the test
results.
\nl

\fix{}{oversigt over test}
