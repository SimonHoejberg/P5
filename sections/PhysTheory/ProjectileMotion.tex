...
Another important point is that the variable $\mathbf{y_0}$ changes dynamically
based on angle of the turret. This is because the muzzle of the turret
changes its veritcal position based on the angle. This can be seen in
\autoref{TurretCalcY}.
\figx[0.2]{TurretCalcY}{Turret marked with the variables A, r, p, h and $y_0$}

In order to calculate $y_0$ for any given angle we can make use of the sine for
the angle. Looking at \autoref{TurretCalcY} we can formulate the following
formula: 
\begin{equation}\label{Y0Eq}
y_0\ =\ h\ +\ r\ *\ sin(a)
\end{equation}
Substituting this into \autoref{FinEq} gives the following formula:
\begin{equation}\label{FinFinEq} 
x\ =\ v_0*cos(a)*\frac{v_0*sin(a)+\sqrt{v_0^2*sin(a)^2-2*g*(h+r*sin(a))}}{g}
\end{equation}
Now, using \autoref{FinFinEq} it is possible to calculate the
...
\autoref{FinFinEq} where $\mathbf{cos(a)}$ and $\mathbf{sin(a)}$ are used as
