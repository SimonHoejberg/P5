...
The equations used for determining the height and length of the shot at any
given time are:
...
\subsection{Calculating length}
Using \autoref{sheight} it is possible to isolate \textbf{t}. This can then
substitute t in \autoref{slength}. This gives the equation:
\begin{equation}
x\ =\ v*cos(a)*\frac{v*sin(a)+\sqrt{v^2*sin(a)^2-2*g*y}}{g}
\end{equation}

This can then be used to calculate the maximum distance the projetile can
travel.

figur af turret og kasteparabel
forklaring af hvordan ændring af hastighed og vinkel påvirker skuddet
\subsection{tid}
noget om tid
\begin{equation}
t=\frac{x}{v_0*cos(a)}
