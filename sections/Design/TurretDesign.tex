...
In the following subsections, arguments for each part of this design
wil be presented.
...
ball launcher, two wheels placed side by side spinning in opposite directions, with the projectile pushed through.\fix{add sketch for a
...
which is seen in \autoref{Accuracy}. This shows that if all 5 projectiles are
fired the turret has good odds of hitting the target.
...
to stop. This is beacause the motor delivers a lot of power, which makes the
turret turn fast, even at low settings for the motor. This means that the turret
is very imprecise which makes it difficult to aim correctly.\nl
