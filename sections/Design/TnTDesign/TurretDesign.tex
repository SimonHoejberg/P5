...
shows that the sensors that \name makes use can not sense any longer than the piston design can shoot, which makes the power that the two other designs has obslete.\nl}{need an update flour test}

The cannon is mounted on a motor that is used to control the vertical angle of the cannon, such that it is able to shoot a target at varying distances. This is done by having a
...
distributes the weight more evenly, which makes the turret more stable. The downside of this design is the access to the LCD screen and buttons is obstructed. As a result the LCD is impossible to read from in praxis, and the buttons are hard to reach, although it is possible. In spite of this, the advantages leads us to choosing this design.
