...
of the network. In this network the sensor observations, \textc{S_0} to
\textc{S_2}, are simplified representations of 3 sensor observations each,
...
considered as an implicit part of the variables, in the sense that \textc{S_0}
would be taken at time 0 while \textc{S_2} would be at time 2. \fix{Time is
...
the dependencies. The nodes \textc{Vector_0} and \textc{Vector_1} are used to
represent to possible directional vectors the target can have. The edges moving
from \textc{Vector_0} to \textc{S_0} and \textc{S_1} represent both of these
nodes are dependant on \textc{Vector_0}. Since the sensor nodes are comprised
of distance, angle and time, we need two of such sets in order to reliably
determine a direction vector. \textc{Vector_1} has edges to \textc{S_1},
\textc{S_2}, this represent that the we use the previous observation to
determine a new direction vector. The edge going from \textc{Vector_0} to
\textc{Vector_1} represent that the previous direction vector can be compare the
new vector to the old.
