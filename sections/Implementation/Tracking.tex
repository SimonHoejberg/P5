\subsection{Tracking}
The first main thread of the program is \textc{Track()}, which is responsible
...
different blobs per frame. This can be seen under \fix{section 1}{} in the code
...
for this feature can be found \fix{under section 2}{} in \autoref{TrackVar}.\nl
...
and the cameras centers is stored in \textc{temp} for use in future
calculations. The boolean on \textbf{line 3} tells that the blob is in the view
of the camera, this boolean is used in the \textc{GetDataAndCalculate} task
...
determined in which direction the target is moving. \fix{Assuming}{assuming
what} this is the case (\textbf{lib\_direction!=0}), the \textc{GetSpeed} function is called in order
...
\textc{MAX\_COUNTER\_TO\_PURGE\_DATA}, after which all values involved
...
the resulting rotating speed. The \fix{sine function maps}{how?} the 144 pixel
width of the camera to 90 degrees for a sine function. This way the speed should follow a
