...
In this section the features and requirements for this project will be
formalized, this is done based on the analysis in \autoref{PlatformC} and
\autoref{SensorTest}.

The following features and requirements were defined for the project:
\begin{itemize}
  ...
  \begin{itemize}
    \item The system needs to use the NXT as the embedded
  ...
  \begin{itemize}
    ...
    target, it needs to be able to turn. If it is not able to turn, the line of
    ...
    to 90 degrees, as this will provide a decently sized area in front of the
    turret.
    \item In order to perceive the target, the turret will need to make use of
    sensors.     
    \item The turret needs to be able to shoot in order to hit a
    target. This has to be done within a range of 200cm as it is the max
    capability of the sensors see \autoref{SensorTest}. 
...
%     updated/changed to the above.}
  ...
  Based on these requirements and the analysis it is now possible to make design
  choices about the construction of \namep, such that it is able to hit a
  target in motion. Based on the tests of the camera and ultrasonic
  sensors, it is possible to construct a target that \name is
  able to detect and hit. 
