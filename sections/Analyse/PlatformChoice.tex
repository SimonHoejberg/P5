...
released by Lego the Robotics Invention System.}{Sales talk and history which
if needed, needs a source} Although the NXT product-line contains the NXT 1.0,
...
unofficial programming languages for the NXT, such as \michael{leJOS(Java),
RobotC(C), nxt-Python(Python-duhh)} which makes it possible to write programs using the more popular programming
languages \Source.
...
\fix{The drawback would be}{Missing sentence?}
...
\begin{enumerate}
  ...
  superior. \fix{Although the Arduino offers a selection of boards with varying
  ...
  neglible.}{Men vi har kun uno til rådighed og derfor kigger vi kun på den som
  vi skriver længere oppe }
...
\begin{itemize}
  ...
  \fix{multiplexer}{What is it?} can allow expansion of existing ports.
\end{itemize}
