...
of solutions \citep[ch.1.4.1]{MIBook}:
...
factors, which are used to give an estimation of the system \citep[ch.1.5]{MIBook}. 
These factors are: modularity, representation scheme, planning
...
\name is only able to partially observe where the target is and how fast it is
moving and will thus need to be able to reason under uncertainty. This means
that it will need a way to consider different possibilities for each world.
This can be done with a belief network which is an acyclic directed graph
consisting of a set of random variables, which is variables which has a domain
that is a set of the values the varibale can have, and a set of conditional
probability distributions for each variable given its parents. From these
conditional probality distributions it is given that evidence is a formula that
represnts the union of all the observations that an agent makes of the world
\citep[ch.6.3]{MIBook}.
These variables and their domains are usually derived from the features in the
representation scheme, although some random variables can be created to act as intermidiate variables. The
...
\begin{itemize}
  ...
  This kind of connection represnts that each node is affected by the node prior
  to it. These connections allow information to flow as long as the middle
  node is not blocked by evidence. If evidence is present on the middle node, it is not
 ...
 \figx[0.8]{Serial2}{An example of a serial connection}
  ...
  node, or one of its descendants, makes it impossible to transmit information between
 ...
 \figx[0.8]{Convergent2}{An example of convergent connection}
  ...
  more children. This kind of connection reprensts that one node affects more
  than one node. In this kind of connection it is only possible to transmit
  information between the children if the parent node has evidence. An example of a divergent connection can be seen in \autoref{Divergent2}.
...
This means that if the node has few parents, the number of variables that can
directly affect it are few. It also means that the fewer parents each node has
the fewer probabilities need to be specified, since all the influence of
variables that are not descendants of a given variable is given when the value
of the variables parents are known.

\subsection{Hidden Markov Model}

A belief network can be made to model a dynamic system by using a \textbf{Markov
chain} to represent a feature at different points in time as random variables.
By adding observations to the Markov chain, it becomes a \textbf{hidden Markov
