...
\citep[ch.1.1]{MIBook} when its decisions are explainable by computations.
How intelligent it is, can be judged by:

\begin{enumerate}
  \item If what it does is appropriate to its circumstances and goals.
  \item If it is flexible to changing environments and changing goals.
  \item If it learns from experience.
  \item If it makes appropriate choices given its perceptual and computational
  limits.
\end{enumerate}
...
directed graph consisting of random variables. These variables and their domain
are usually derived from the features in the representation scheme. The
...
evidence on a node.
\fix{}{Update nodes such that they are consistent with example below}
\begin{itemize}
  ...
  \autoref{serial.JPG}.
 \figx{serial.JPG}{A serial connetion with evidence on the middle node}
 ...
 \autoref{convergentnoevidence.JPG}.
 \figx{convergentnoevidence.JPG}{A convergent connection with no evidece on the
 child node}
  ...
  example of a divergent connection can be seen in \autoref{divergent.JPG}.
 \figx{divergent.JPG}{A divergent connection with evidence on the parent node}
\end{itemize}
...
This means that if the number of parents each node has is low the number of
variables that is affected by one another is small. This means that the fewer
parents each node has the fewer probabilities need to be specified.
With this assumption and the connections described above, we are able to find
the conditional probabilities of the belief network.
...
of parameters can provide an infinite network\citep[Ch.6.5.1]{MIBook}.
For example: A man observes an airplane taking off northbound at $S_0$, then at
$S_1$ he observes that the airplane has turned eastward, at $S_2$ he observes
that it has turned southward and at $S_3$ he sees that it continues south. This
allows him to guess the position of the plane at $S_4$, see\ldots
\fix{}{Insert example of HMM}
