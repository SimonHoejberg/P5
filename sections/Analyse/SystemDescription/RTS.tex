...
\begin{itemize}
  ...
  \item \textbf{Aperiodic} tasks are those which are activated by an ooccurance
  of an event.
\end{itemize}

Using the previous example, it is clear that the system with the ABS would not
be able to function since a failure to complete the task may result in the wheels locking which in turns causes an accident.\nl
...
\begin{itemize}
  ...
  be unusable, the consequence of missing it is not very severe.
  \item \textbf{Soft} missing this deadline is not severe at all, and the
  usefullnes of the result decrease after the deadline has passed.
\end{itemize}
